      ...
      <div style="text-align: center" class="card">
        ...
        <div class="card-body">
          <form @submit.prevent="submit()">
            <input v-model="subject" required type="text" class="form-control"><br>
            <button class="btn btn-dark" style="margin: 5px" href="/add-keyword">اضافه کردن  </button>
            <a href="/news" class="btn btn-warning" style="margin: 5px">بازگشت به صفحه اخبار  </a>
          </form>
...
export default {
  ...
  methods: {
    ...
    async submit(){
      await axios
      .post('intrests', {subject:this.subject})
      .then(response => response.data)
      .then(response => {
        this.reports = response
      })
