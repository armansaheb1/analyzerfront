<template>
  ...
  <div>
    <div style="text-align: right; max-width: 100%; overflow-x: auto;margin-right: 10px;font-family: 'vazir';">
      <div v-if="!$store.state.isAuthenticated" style="text-align: center">
        ...
        <br>
        <br>
        <a class="btn btn-dark"style="margin: 5px" href="/register">ثبت نام</a><a class="btn btn-dark"style="margin: 5px" href="/login">ورود</a> 
      </div>
    ...
    <div style="max-width: 100%;overflow-x: auto;overflow-y: hidden;height: 65px;padding-top: 5px;direction: rtl
  ">
  <div v-for="itemm in services" style="text-align: right; max-width: 100%;display: inline-block;direction: rtl;margin-top: 10px;font-family: 'vazir';">
    <a style="text-decoration: none" :href="'/news-services/' + item.id + '/' + itemm.id">
  ...
  <div class="alert alert-warning" v-if="!reports.length && !noint && $store.state.isAuthenticated">
